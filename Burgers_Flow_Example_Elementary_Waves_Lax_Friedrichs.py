...
def exactSolutionOutEdgeThree(x: float, t: float) -> float:
  if 0. <= x < np.sqrt(2./3.)*t :
    return np.sqrt(2./3.)
  elif np.sqrt(2./3.)*t <= x < 2*t:
    return x/t
  else:
    return 2.0
...
burgersNumerical = NumericalFlux(name='Burgers Flow Numerical', fluxFunction=fluxcollection.laxFriedrichs, analyticalFlux=fluxcollection.positiveBurgers, analyticalDerivative=fluxcollection.derivativePositiveBurgers, capacity=1.)
...
streetsPlot.showAnimations('Burgers Elementary Waves')
streetsPlot.saveAnimation3d('Burgers_Flow_Elementary_Waves_Lax_Friedrichs_Example')
# streetsPlot.plot3D(0, 't=0.0')
# streetsPlot.plot3D(discreteEquation.getNumTimeSteps()-1, 't=0.3')
